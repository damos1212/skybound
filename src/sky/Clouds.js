...
// Adapted from Tidewater's SkyProClouds (MIT) for a camera that climbs through and above the layer:
// the shape noise is generated on the GPU at start-up, the panorama covers the whole sphere, nothing
// is masked below the horizon, and `compositeModule` blends the view's clouds over the scene by depth
// (so the deck hides the sea when flying above it). Original notes follow.
//
...
// sky-pro "high" quality, and cheaper tiers for smaller GPUs
const QUALITIES = {
	high: { historyDivisor: 2, lattice: 2, maxSteps: 192, lightTaps: 6, stepMeters: 25, fullLightingAlpha: 0.5, lightReuseSteps: 3, historyWeight: 0.9 },
	medium: { historyDivisor: 2, lattice: 4, maxSteps: 160, lightTaps: 5, stepMeters: 36, fullLightingAlpha: 0.45, lightReuseSteps: 4, historyWeight: 0.9 },
	low: { historyDivisor: 3, lattice: 4, maxSteps: 96, lightTaps: 4, stepMeters: 55, fullLightingAlpha: 0.4, lightReuseSteps: 5, historyWeight: 0.92 },
};
let QUALITY = QUALITIES.high;

const PANO_W = 512, PANO_H = 256; // whole sphere, elevation linear in v (-90 at the bottom row)
...
export class Clouds {
	...
	constructor( renderer, atmosphere, { quality = 'high' } = {} ) {

		this.renderer = renderer;
		this.atmosphere = atmosphere;
		QUALITY = QUALITIES[ quality ] || QUALITIES.high;
		...
		// ---- textures
		this.noise = new Texture( { label: 'cloud shape noise 64³', width: 64, height: 64, depth: 64, dimension: '3d', format: 'rgba8unorm', mips: true, usage: [ 'sample', 'storage' ] } );
		...
		this.viewMeta = this.metas[ 0 ];

		this.ready = this._generateNoise();
		this._buildModules();
		this._buildKernels();
	...
	// 64³ tileable Perlin-Worley / Worley fBm volume (r: Perlin-Worley, g / b / a: Worley fBm at 4, 8,
	// 16 cells), box-filtered mips, and a 64² interleaved-gradient dither for the ray start.
	async _generateNoise() {

		const t = this.noise;
		const NOISE = /* wgsl */`
fn nzHash( p: vec3i ) -> vec3f { return vec3f( pcg3( bitcast<vec3u>( p ) ) ) * ( 1.0 / 4294967295.0 ); }
fn nzWrap( p: vec3i, period: i32 ) -> vec3i { return ( ( p % period ) + period ) % period; }
// 1 at the feature points, 0 one cell away
fn nzWorley( p: vec3f, period: i32 ) -> f32 {
	let id = vec3i( floor( p ) );
	let f = fract( p );
	var md = 1e9;
	for ( var z = -1; z <= 1; z++ ) { for ( var y = -1; y <= 1; y++ ) { for ( var x = -1; x <= 1; x++ ) {
		let o = vec3i( x, y, z );
		let d = vec3f( o ) + nzHash( nzWrap( id + o, period ) ) - f;
		md = min( md, dot( d, d ) );
	} } }
	return 1.0 - clamp( sqrt( md ), 0.0, 1.0 );
}
fn nzGrad( c: vec3i, period: i32, f: vec3f ) -> f32 {
	let h = nzHash( nzWrap( c, period ) + vec3i( 71, 13, 37 ) ) * 2.0 - 1.0;
	return dot( normalize( h + vec3f( 1e-4 ) ), f );
}
fn nzPerlin( p: vec3f, period: i32 ) -> f32 {
	let i = vec3i( floor( p ) );
	let f = fract( p );
	let u = f * f * f * ( f * ( f * 6.0 - 15.0 ) + 10.0 );
	let a = mix( nzGrad( i, period, f ), nzGrad( i + vec3i( 1, 0, 0 ), period, f - vec3f( 1.0, 0.0, 0.0 ) ), u.x );
	let b = mix( nzGrad( i + vec3i( 0, 1, 0 ), period, f - vec3f( 0.0, 1.0, 0.0 ) ), nzGrad( i + vec3i( 1, 1, 0 ), period, f - vec3f( 1.0, 1.0, 0.0 ) ), u.x );
	let c = mix( nzGrad( i + vec3i( 0, 0, 1 ), period, f - vec3f( 0.0, 0.0, 1.0 ) ), nzGrad( i + vec3i( 1, 0, 1 ), period, f - vec3f( 1.0, 0.0, 1.0 ) ), u.x );
	let d = mix( nzGrad( i + vec3i( 0, 1, 1 ), period, f - vec3f( 0.0, 1.0, 1.0 ) ), nzGrad( i + vec3i( 1, 1, 1 ), period, f - vec3f( 1.0, 1.0, 1.0 ) ), u.x );
	return mix( mix( a, b, u.y ), mix( c, d, u.y ), u.z );
}
fn nzWorleyFbm( uvw: vec3f, freq: i32 ) -> f32 {
	return nzWorley( uvw * f32( freq ), freq ) * 0.625 + nzWorley( uvw * f32( freq * 2 ), freq * 2 ) * 0.25 + nzWorley( uvw * f32( freq * 4 ), freq * 4 ) * 0.125;
}
fn nzPerlinFbm( uvw: vec3f, freq: i32 ) -> f32 {
	var sum = 0.0; var amp = 1.0; var norm = 0.0; var fr = freq;
	for ( var o = 0; o < 5; o++ ) {
		sum += nzPerlin( uvw * f32( fr ), fr ) * amp;
		norm += amp; amp *= 0.5; fr *= 2;
	}
	return sum / norm * 0.5 + 0.5;
}
`;
		const gen = new ComputeKernel( {
			label: 'Cloud Noise Generate',
			modules: [ commonModule ],
			bindings: { nzOut: { storageTexture: t, access: 'write', view: { dimension: '3d', baseMipLevel: 0, mipLevelCount: 1 } } },
			workgroupSize: [ 4, 4, 4 ],
			code: NOISE + /* wgsl */`
@compute @workgroup_size( WG_X, WG_Y, WG_Z ) fn main( @builtin( global_invocation_id ) id: vec3u ) {
	let uvw = ( vec3f( id ) + 0.5 ) / 64.0;
	let w4 = nzWorleyFbm( uvw, 4 );
	let perlin = nzPerlinFbm( uvw, 4 );
	// Perlin-Worley (Schneider 2015): billowy Perlin with Worley cells carved in
	let pw = clamp( ( perlin - ( w4 - 1.0 ) ) / ( 1.0 - ( w4 - 1.0 ) ), 0.0, 1.0 );
	textureStore( nzOut, id, vec4f( clamp( pw * 1.25 - 0.2, 0.0, 1.0 ), w4, nzWorleyFbm( uvw, 8 ), nzWorleyFbm( uvw, 16 ) ) );
}
`,
		} );
		gen.dispatch( [ 16, 16, 16 ] );
		const levels = t.mipLevelCount;
		for ( let m = 1; m < levels; m ++ ) {

			const size = Math.max( 1, 64 >> m );
			const k = new ComputeKernel( {
				label: 'Cloud Noise Mip ' + m,
				bindings: {
					nzSrc: { texture: t, view: { dimension: '3d', baseMipLevel: m - 1, mipLevelCount: 1 } },
					nzDst: { storageTexture: t, access: 'write', view: { dimension: '3d', baseMipLevel: m, mipLevelCount: 1 } },
				},
				workgroupSize: [ 4, 4, 4 ],
				code: /* wgsl */`
@compute @workgroup_size( WG_X, WG_Y, WG_Z ) fn main( @builtin( global_invocation_id ) id: vec3u ) {
	if ( any( id >= vec3u( ${ size }u ) ) ) { return; }
	var s = vec4f( 0.0 );
	for ( var i = 0u; i < 8u; i++ ) {
		let o = vec3u( i & 1u, ( i >> 1u ) & 1u, ( i >> 2u ) & 1u );
		s += textureLoad( nzSrc, id * 2u + o, 0 );
	}
	textureStore( nzDst, id, s * 0.125 );
}
`,
			} );
			k.dispatch( [ Math.ceil( size / 4 ), Math.ceil( size / 4 ), Math.ceil( size / 4 ) ] );

		}

		const blue = new Uint8Array( 64 * 64 );
		for ( let y = 0; y < 64; y ++ ) for ( let x = 0; x < 64; x ++ ) {

			const v = 52.9829189 * ( ( 0.06711056 * x + 0.00583715 * y ) % 1 );
			blue[ y * 64 + x ] = Math.floor( ( v % 1 ) * 255 );

		}

		this.blue.upload( blue );
		GPU.submit();
...
fn cloudsSample( dir: vec3f ) -> vec4f {
	...
	let v = clamp( elev / PI + 0.5, 0.0, 1.0 );
	return textureSampleLevel( cloudsPanorama, smpLinearRepeat, vec2f( u, v ), 0.0 );
}
...
fn cloudsSampleView( dir: vec3f ) -> vec4f {
	...
	if ( inside ) {
		...
		return vec4f( v.rgb, 1.0 - min( v.a, 1.0 ) );
	}
...
`,
		...
		// the view's clouds over scene geometry: vec4( in-scattered radiance, transmittance ), only where
		// the cloud's (weighted) depth is in front of the scene point `sceneDist` (m) away
		this.compositeModule = new ShaderModule( {
			name: 'cloudsComposite',
			deps: [ this.module ],
			bindings: { cloudsViewMeta: { texture: () => this.viewMeta } },
			code: /* wgsl */`
fn cloudsComposite( dir: vec3f, screenUv: vec2f, sceneDist: f32 ) -> vec4f {
	if ( cloudsParams.viewValid < 0.5 ) { return vec4f( 0.0, 0.0, 0.0, 1.0 ); }
	let x = dot( dir, cloudsParams.viewRight );
	let y = dot( dir, cloudsParams.viewUp );
	let z = dot( dir, cloudsParams.viewFwd );
	let uv = vec2f( x / max( z, 1e-4 ) / cloudsParams.viewTan.x * 0.5 + 0.5, 0.5 - y / max( z, 1e-4 ) / cloudsParams.viewTan.y * 0.5 );
...
${ MAIN } {
	...
	let m = scMarch( scf.position.xyz, dir, dither, pixelConeAngle, stepConeAngle, u32( scf.march.y ), scf.display.z > 0.5 );
...
${ MAIN } {
	...
	update( dt, camera, originY = 0, originX = 0 ) {
		...
		F.display.value = [ 0, q.lightReuseSteps, this.useBounds === false ? 0 : 1, 0 ];
		...
		if ( underwater ) {
			...
			this.viewMeta = this.metas[ this._pp ];
			this._pp = 1 - this._pp;
